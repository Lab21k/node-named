...
Server.prototype.listen = function listen(port, address, callback) {
        ...
        var protocolVersion = net.isIP(address);
        if (protocolVersion === 0) {
            self.emit('error', new Error('Invalid IP Address (not IPV4 or IPV6)'));
        }

        var udpVersion = protocolVersion === 4 ? 'udp4' : 'udp6';

        this._socket = dgram.createSocket(udpVersion);
